...
const FALLBACK_STORY_IMG: PageImage = {
...
function StorySection({ mainImage = FALLBACK_STORY_IMG }: { mainImage?: PageImage }) {
    const imgSrc = mainImage.url || FALLBACK_STORY_IMG.url;
    const imgAlt = mainImage.alt || FALLBACK_STORY_IMG.alt;
    return (
...
export function AboutContent({ mainImage }: { mainImage?: PageImage }) {
    return (
        <div className="w-full overflow-x-hidden bg-cream-50">
            ...
            <StorySection mainImage={mainImage} />
            <VisionSection />
            <QuoteSection />
        </div>
    );
